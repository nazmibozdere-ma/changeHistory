...
import { storefrontOptions } from '../data/mockData';
...
export default function CampaignSelectorModal({ campaigns, selected, onConfirm, onCancel, asModal = false }: CampaignSelectorProps) {
  ...
  const [storefrontFilter, setStorefrontFilter] = useState('All Storefronts');
  ...
  const visibleCampaigns = campaigns.filter(c => {
    ...
    const matchesStorefront =
      storefrontFilter === 'All Storefronts' || c.storefront === storefrontFilter;
  ...
  const panel = (
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg flex flex-col" style={{ width: '360px', maxHeight: '480px' }}>
      ...
      <div className="px-3 pt-3 pb-2 space-y-2">
        ...
        {/* Status + Storefront filters */}
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <button
              onClick={() => { setStatusOpen(o => !o); setStorefrontOpen(false); }}
              className="flex items-center justify-between gap-1 w-full px-2.5 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors whitespace-nowrap"
            >
          ...
          <div className="relative flex-1">
            <button
              onClick={() => { setStorefrontOpen(o => !o); setStatusOpen(false); }}
              className="flex items-center justify-between gap-1 w-full px-2.5 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors whitespace-nowrap"
            >
              <span className="truncate">{storefrontFilter}</span>
              <ChevronDownIcon className="text-gray-400 shrink-0" />
            </button>
            {storefrontOpen && (
              <div className="absolute right-0 top-full mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                {storefrontFilterOptions.map(opt => (
                  <button
                    key={opt}
                    onClick={() => { setStorefrontFilter(opt); setStorefrontOpen(false); }}
                    className={`flex items-center justify-between w-full px-3 py-1.5 text-xs text-left hover:bg-gray-50 transition-colors ${storefrontFilter === opt ? 'text-blue-600 font-medium' : 'text-gray-700'}`}
                  >
                    <span className="truncate">{opt}</span>
                    {storefrontFilter === opt && <CheckIcon className="text-blue-600 shrink-0" />}
                  </button>
                ))}
              </div>
            )}
